...
func IgPostCreate(ctx context.Context, input model.NewIgPost) (*model.IgPost, error) {
	if len(input.Files) == 0 {
		fmt.Println("Files Len : 0")
		return nil, &gqlerror.Error{
			Message: "Uploaded File Must Not Empty",
...
//IgPostGetByUserID Get By UserID
func IgPostGetByUserID(ctx context.Context, userID int) ([]*model.IgPost, error) {
	db := config.ConnectGorm()
	sqlDB, _ := db.DB()
	defer sqlDB.Close()

	var igPosts []*model.IgPost

	err := db.Table("ig_post").Where("user_id = ?", userID).Find(&igPosts).Error
...
//IgPostGetByToken Get By Token
func IgPostGetByToken(ctx context.Context) ([]*model.IgPost, error) {
	userID := ForContext(ctx).ID
	return IgPostGetByUserID(ctx, userID)
}
